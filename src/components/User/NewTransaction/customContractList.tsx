...
const CustomContractDetails = (props: any) => {
  ...
  return (
    <div>
      <div className="stepDetails mt-3 mb-4">Custom contract details</div>
      <Card className="grayCard pt-3">
        <Row gutter={{ md: 36, sm: 16, xs: 16 }} >
          {customAttachmentUrls?.length ? 
            customAttachmentUrls.map((attachmentUrl:string) => 
              (<CustomAttachments 
                key={attachmentUrl}
  ...
  return <> 
    <Col style={{marginBottom: '10px'}}>
      ...
      {!customAttachUrl ? null : customAttachUrl.includes(".pdf") ? (
        <div className="signature-image pdf-viewIcon m admin-panel-pdf-preview mx-3">
          <PDFPreview
            url={customAttachUrl}
