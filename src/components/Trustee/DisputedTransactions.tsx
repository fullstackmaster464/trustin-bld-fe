...
import { DisputeTrusteeTxn, searchTrusteeTxn } from "../../services/trustee";
// import { LoadingOutlined } from "@ant-design/icons";
import { createPdf } from "../User/NewTransaction/pdfGeneratorHelper";

const DisputedTransaction = ({searchedKey, setSearchedKey}:any) => {
    ...
    allLocale: {
      emptyText: (
        <>
          ...
          <p className="nodata my-5">No disputed agreements found</p>
        </>
      ),
    },
  };
      ...
      render: (text: any) => {
        return (
          <Typography.Text ellipsis={true} style={{ maxWidth: 150 }}
            className="hyperLink"
            onClick={() => {
              navigate(TransactionDetail + "/" + text?.aliasName, {
                state: { pagefrom: "pending" },
              });
            }}
          >
            {text?.agreementId}
  ...
  const DisputeTxn = (current: number, page: number) => {
    setLoading(true);
    DisputeTrusteeTxn(current || 1, page || 10, "sortByAgreementId", "DESC", {
      trusteeId: userAlias,
      id: "dispute",
    })
      .then((response) => {
  ...
  return (
    <div>
      <div className="fullHeight">
        ...
        <Modal
          title={
            <div className="titleText mt-3 mb-5">Contract details PDF</div>
          }
          centered
          open={isModalOpen}
...
export default DisputedTransaction;
