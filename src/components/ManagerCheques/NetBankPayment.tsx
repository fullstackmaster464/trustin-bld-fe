...
import { getTransactionLinkList, getUaepgsFormDataUrl, virtualAccountDetails } from "../../services/admin";
...
import { getChequeDetails } from "../../services/cheque";
...
const NetBankPayment = (): any => {
  ...
  const openNotification = (msg = "") => {
      ...
      if(chequeAlias){
        getChequeDetails(chequeAlias, userAlias)
          .then(async (response: any) => {
            setchequeDetail(response?.data);
            console.log(response.data)
            const buyerAmount = response.data.buyerAmount;
            getVaTransactionList(chequeAlias, buyerAmount);
          })
          .catch((error: any) => {
    ...
    const getVaTransactionList= async (chequeAlias: string, buyerAmount: any) => {
        try {
          if(!chequeAlias) {
            return;
          }
          const response = await getTransactionLinkList(chequeAlias);
          ...
          const transactionAmount = Number(buyerAmount || 0);
          const requiredAmount = transactionAmount - totalAmount;
    ...
    useEffect(() => {
      if(userAlias && chequeDetail?.currency) {
        virtualAccountDetails(userAlias, chequeDetail?.currency)
          .then((response) => {
    ...
    }, [userAlias, chequeDetail])
    const goBack = () => {
      navigate(Cheques + "/" + chequeAlias);
    };
    ...
    const validateAmount = (e: any) => {
      ...
      if (requiredAmountToBeAdded && amount > parseFloat(requiredAmountToBeAdded)) {
        setAmountErrorMessage("Amount must be less than or equal to left total amount");
        setAmountError(true);
    ...
    const handelSubmit = async (event: any) => {
      ...
      if (selectedBankId) {
        ...
        if (!description) {
          desc = chequeDetail?.agreementId
        }
        console.log(requiredAmountToBeAdded,selectedAmount);
        ...
        const billReference = chequeDetail?.agreementId.replace(/-/g, '.');
        ...
        >
          ...
          <div className="">
            <div className="subText_xs w-100 paymentMethods pl-5">
              <p>Note: Extra charge of {process.env.UAEPGS_PLATFORM_FEE} {chequeDetail?.currency} + {process.env.COUNTRY_VAT}% vat is applied on every transaction.</p>
            </div>
          </div>
                  ...
                  <div>
                    ...
                    <Form.Item
                      ...
                      rules={[
                        {
                          validator(_, value) {
                            if (parseFloat(value) > requiredAmountToBeAdded) {
                              return Promise.reject(`Amount must be less than or equal to left total amount`);
                            } else {
              ...
              {moneyFormat(
                ...
                <div className="endtoend py-2">
                  ...
                  <div className="subText_small fw-400 text-right">
                    {moneyFormat(
                      chequeDetail.currency,
                      parseFloat(chequeDetail.invoiceAmount).toFixed(2)
                    )}
                  </div>
                </div>
                <div className="endtoend py-2">
                  <div className="stepDetails_medium_sub">
                    ...
                    {`(${chequeDetail.platformFeePercent})`}
                  </div>
                  <div className="subText_small fw-400 text-right">
                    {moneyFormat(
                      chequeDetail.currency,
                      chequeDetail.platformFee
                    )}
                  </div>
                </div>
                <div className="endtoend py-2">
