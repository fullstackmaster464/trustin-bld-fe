import {
  ...
  import {
    ...
    getItemTypeFilter,
    getRealEstateType,
    getRealEstateTypeByEntity,
    realestatetypecategory,
    searchRSItemType, 
  ...
  const RealestateItemType = ():any => {
    ...
    const [ItemName, setCurrentItemName] = useState("");
    ...
    useEffect(() => {
      if (isEditModalVisible && itemDetails) {
        ...
        setentityType(itemDetails?.categoryentitytype);
    ...
    const handleChange = (value: number) => {
      ...
      else if (Object.keys(searchedKey)?.length > 0 && (searchedKey.name || searchedKey.itemCategory || searchedKey.status)) {
        handleFilterApply(searchedKey,1,value)
      }
       else {
        fetchRealEstateItemType(1, value);
      }
    };
    const setWidthVal = () =>{
    ...
    const onChangePage = (pageno: number) => {
      ...
      if (Object.keys(searchedKey)?.length > 0) {
        ...
        fetchRealEstateItemType(pageno, page);
      }
    };
    const handleCancel = () => {
    ...
    const onFinish = (values: any) => {
      ...
      createRealEstateType(values)
        .then((response) => {
          if (response?.status === 201) {
            setIsAddModalVisible(false);
            fetchRealEstateItemType(1, page);
            form.resetFields()
          }
          form.resetFields();
        })
    ...
    const removeRealEstateItemType = () => {
      setLoading(true);
      deleteRealEstateType(itemDetails?.aliasName).then(() => {
        fetchRealEstateItemType(current, page);
        setItemDetails({});
        setIsReloadModalVisible(false);
        setIndex("");
        setLoading(false);
        message.success("Manager cheque type deleted successfully");
      })
      .catch((error) => {
        setLoading(false);
        setIsReloadModalVisible(false);
        message.error("Failed to delete item. Please try again later.");
        console.error("Delete error:", error);
      });
    };
    const updateRealEstateItemType = () => {

      if (!ItemName) {
        setEditErrorMessage("Item type name is required!");
        return;
      }
      
      const params = {
        categoryAlias: itemDetails?.aliasName,
        name: ItemName,
      ...
      setLoading(true);
      updateRealEstateType(itemDetails?.aliasName, params)
        .then(() => {
          setIndex("");
          setCurrentItemName("");
          setItemDetails({});
          ...
          handleCancel();
          setLoading(false);
          message.success("Manager cheque type updated successfully");
        })
        .catch((error) => {
          setLoading(false);
          if (error?.response?.status === 400) {
            message.error("Oops! Could not update item. Please try again later");
          } else {
    ...
    const editType = (value : any)  => {
      
      handleEntityType(value?.entityType);
      setIsEditModalVisible(true);
      setItemDetails(value);
      setCurrentItemName(value?.name);
      setIndex(index);
    }
    ...
    const updateStatus = (values: any, status: string, index: number) => {
      ...
      setLoading(true);
      updateRealEstateType(values?.aliasName,params).then(() => {
        setLoading(false);
    ...
    const onTabChange = (entityType ="") => {
      setLoading(true)
      getRealEstateTypeByEntity(current, page,entityType).then((res : any)=> {
        setLoading(false)
    ...
    const fetchRealEstateItemType = (_current: number, page: number, aliasname = "") => {
      setLoading(true);
      getRealEstateType(aliasname, _current, page)
        .then((response) => {
          setLoading(false);
        ...
        .catch(() => {
          setLoading(false);
          message.error("Could not fetch category details. Please try again later");
        });
      ...
      getRealEstateType(aliasname, _current, page)
        ...
        .catch(() => {
          setLoading(false);
          message.error("Could not fetch type details. Please try again later");
        });
    };
    const onSearch = (e: string, currentPage: number, page: number, entityType="") => {
      ...
      if (e) {
        setSearchedKey(e);
        const reqBody = { key: e };
        searchRSItemType(currentPage > 0 ? currentPage - 1 : 0, page || 10, reqBody)
          .then((response) => {
            setLoading(false);
            setItemCategoryList(response.data.data);
            setCount({
        ...
        setSearchedKey("");
        fetchRealEstateItemType(1, page, entityType);
      }
    };
    useEffect(() => {
      fetchRealEstateItemType(current, page);
  ...
  const handleFilterApply = (e:any, current :number, pageSize:number) =>{
    ...
    getItemTypeFilter(current > 0 ? current -1 : 0,
      ...
      .catch(() => {
        setLoading(false);
        message.error("Could not fetch filtered data. Please try again later");
      });
  }
    return (
      <div className="scrollbar-container">
        ...
        <DefaultLayout
          page="config_type_real"
          // loading={loading}
          TitleText="Manager cheque types"
            ...
            <div className="d-flex">
                      ...
                      <div className="ml-5">
                        ...
                        <Breadcrumb separator=">">
                          ...
                          <Breadcrumb.Item>Manager cheque types</Breadcrumb.Item>
                        </Breadcrumb>
                      </div>
                    </div>
                  }
        >
                <Card className="noBorder transparent mt-6">
                  ...
                  />
                    ...
                    <Modal
                      title={<p className="large-title">Add New Type</p>}
                      ...
                      <Col span={24}>
                        ...
                        >
                          ...
                          <Row gutter={16}>
                            <Col span={Width > 991 ? 12 : 24}>
                              ...
                              <Form.Item
                                ...
                                rules={[
                                  {
                                    required: true,
                                    message: "Manager cheque category is required!",
                                  },
                                ]}
                              >
                                <Select
                                  className=""
                                  placeholder="Select manager cheque category"
                            ...
                            <Col span={Width > 991 ? 12 : 24}>
                              <p className="enter-text-category">Type name</p>
                              <Form.Item
                                name="name"
                                ...
                                rules={[
                                  {
                                    required: true,
                                    message: "Manager cheque type is required!",
                                  },
                                  {
                                    whitespace: true,
                                    message: "Enter valid manager cheque type!",
                                  },
                                ]}
                              >
                                <Input placeholder="Enter manager cheque type name" onInput={handleCategoryNameChange}></Input>
                              </Form.Item>
                            </Col>
                          </Row>
                    ...
                    <Modal
                      title={<p className="large-title">Edit manager cheque types</p>}
                      className="add-item-category-modal d-flex center"
                      ...
                      <Row gutter={16} className="mt-3">
                        <Col span={Width > 991 ? 12 : 24}>
                          ...
                          <Form.Item name="title" className="modal_inputField">
                            <Select
                              placeholder={itemDetails?.categoryname}
                              disabled
                            ></Select>
                          </Form.Item>
                        </Col>
                        <Col span={Width > 991 ? 12 : 24}>
                          ...
                          <Form.Item className="inputField w-100 error-input">
                            <Input 
                              placeholder="Manager cheque type name" 
                              value={ItemName}
                            ...
                            {!ItemName ? (
                              <p className="errMsg">Please enter manager cheque type name</p>
                              ) : (
                                ""
                            )} 
                        </Col>
                      ...
                      {(ItemName &&
                        itemDetails?.name != ItemName) || (plateformFees && initialValues?.plateformFees != plateformFees) ||
                    ...
                    <Modal
                      title={
                        <div className="modal-title">
                          <p className="large-title center">Manager cheque type</p>
                    ...
                    >
                      <p className="sub-text fw-400 center">
                        Do you want to delete "{itemDetails?.name}" ?
                      </p>
                      <div className="d-flex center">
                        <Button
                          className="modal-button mx-3 my-4"
                          htmlType="submit"
                          onClick={() => removeRealEstateItemType()}
                        >
                          Yes
                        </Button>
                        <Button
  ...
  export default RealestateItemType;
