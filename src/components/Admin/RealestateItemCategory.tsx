...
import {
  ...
  deleteRealEstateCategory,
  getItemCategoryFilter,
  getRealEstateCategory,
  getRealEstateCategoryByEntity,
  searchRSCategoryType, 
...
const RealestateItemCategory = ():any => {
  ...
  const handleChange = (value: number) => {
    ...
    if (typeof searchedKey === 'string' && searchedKey?.length > 0) {
      ...
      fetchRealEstateItemCategory(1, value);
    }
  };
  ...
  const onChangePage = (pageno: number) => {
    ...
    if (Object.keys(searchedKey)?.length > 0) {
      ...
      fetchRealEstateItemCategory(pageno, page);
    }
  };
  const updateRealEstateItemCategory = () => {
    const params = {
      // itemTypeAlias: itemDetails?.aliasName,
      name: ItemUpdatedValue,
      entityType : entityValue
    };
    updateRealEstateCategory(itemDetails?.aliasName, params)
      .then(() => {
        fetchRealEstateItemCategory(current, page);
        ...
        setIndex("");
        setLoading(false);
        message.success("Manager cheque category updated successfully");
      })
      .catch((error) => {
  ...
  const createRealEstateItemCategory = () => {
    if (!ItemUpdatedValue) {
      ...
      createRealEstateCategory(params).then(() => {
        fetchRealEstateItemCategory(current, page);
        setItemDetails({});
      ...
      .catch((error) => {
        ...
        if (error?.response && error?.response?.status === 409) {
          message.error("Manager cheque category with this name already exists")
        } else {
          message.error(err || "Failed to create manager cheque category")
        }
      });
    }
  };
  const removeRealEstateItemCategory = () => {
    setLoading(true);
    deleteRealEstateCategory(itemDetails?.aliasName).then(() => {
      fetchRealEstateItemCategory(current, page);
      setItemDetails({});
      setIsDeleteModalVisible(false);
      setIndex("");
      setLoading(false);
      message.success("Manager cheque category deleted successfully");
    })
    .catch((error) => {
      setLoading(false);
      setIsDeleteModalVisible(false);
  ...
  const updateStatus = (values: any, status: string, index: number) => {
    ...
    setLoading(true);
    updateRealEstateCategory(values?.aliasName,params).then(() => {
      setLoading(false);
  ...
  const fetchRealEstateItemCategory = (_current: number, page: number, aliasname= "") => {
    setLoading(true);
    getRealEstateCategory(aliasname, _current, page)
      .then((response) => { 
        setLoading(false);
  ...
  const onTabChange = (entityType: any) => {
    setLoading(true)
    getRealEstateCategoryByEntity(current, page,entityType).then((res : any)=> {
      setLoading(false)
  ...
  const handleTabChange = (key: string) => {
    ...
    if (searchedKey) {
      ...
      fetchRealEstateItemCategory(1, page);
    }
  };
  ...
  const onSearch = (e: string, currentPage: number, page: number) => {
    ...
    if (e) {
      setSearchedKey(e);
      const reqBody = { key: e };
      searchRSCategoryType(currentPage > 0 ? currentPage - 1 : 0, page || 10, reqBody)
        .then((response) => {
          setLoading(false);
          setItemTypeList(response.data.data);
          setCount({
...
// }
  return (
    <div className="scrollbar-container">
      ...
      <DefaultLayout
        page="config_category_real"
        // loading={loading}
        TitleText="Manager cheque categories"
        ...
        headerPage={
          ...
          <div className="ml-5">
            ...
            <Breadcrumb separator=">">
              ...
              <Breadcrumb.Item>Manager cheque categories</Breadcrumb.Item>
            </Breadcrumb>
          </div>
        </div>
      }
              ...
              <Card className="noBorder transparent mt-6">
                <div className={Width > 1300 ? "w-100 endtoend" : "w-100 endtoend flex-column-reverse"}>
                  ...
                  {Width > 767 && UserType !== "SUPPORT_ENGINEER" ? 
                    <Button
                      className="add-itemtype addBtn w-auto"
                      onClick={() => {
                        setIsAddModalVisible(true);
                      }}
                ...
                {ItemTypeList?.length > 0 ? (
                  ...
                  <Modal
                    title={
                      <div className="modal-title">
                        <p className="large-title center">Manager cheque category</p>
                  ...
                  >
                    <p className="sub-text fw-400 center">
                      Do you want to delete "{ItemUpdatedValue}" ?
                    </p>
                    <div className="d-flex center">
                      <Button
                        className="modal-button mx-3 my-4"
                        htmlType="submit"
                        onClick={() => removeRealEstateItemCategory()}
                      >
                        Yes
                      </Button>
                      <Button
                  ...
                  {/* Create new item */}
                  <Modal
                    title={<p className="large-title">Add manager cheque category</p>}
                    className="modal-box"
                  ...
                  >
                    ...
                    <div>
                      <p className="enter-text">Enter manager cheque category</p>
                      <TextArea
                        rows={4}
                        placeholder="Type your manager cheque name here"
                  ...
                  <Modal
                    title={<p className="large-title">Edit manager cheque category</p>}
                    className="modal-box"
                  ...
                  >
                    <hr className="break-line" />
                    <div>
                      <p className="enter-text">Manager cheque category</p>
                      <TextArea
                        rows={4}
                        placeholder="Type your manager cheque name here"
                        className="modalTextArea mt-2 pt-2 "
                      ...
                      />
                      {!ItemUpdatedValue ? (
                        <p className="errMsg">Please type manager cheque name</p>
                      ) : (
                        ""
                      )}
                      <Row gutter={16} className="mt-3">
...
export default RealestateItemCategory;
